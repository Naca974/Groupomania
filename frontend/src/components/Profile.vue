      ...
      <div class="profil-resum">
        ...
        <div class="mb-3">
            <label for="validPassword" class="form-label white"
              >Biography</label
            >
        <textarea v-model="bio" class="form-control" id="contenu" rows="3" placeholder="Votre Biography"></textarea>
        </div>
        <div class="flex-cont mb-5">
            <button type="submit" class="btn main-btn d-flex m-auto"
  ...
  methods: {
    ...
    updateProfile() {
        ...
        if(this.isUpdate) {
            ...
            axios.put(process.env.VUE_APP_API_BACKEND_URL+'user/profile', formData,
                {headers : {
            ...
            axios.put(process.env.VUE_APP_API_BACKEND_URL+'user/profile', user,
                {headers : {
    ...
    getProfile() {
        axios.get(process.env.VUE_APP_API_BACKEND_URL+'user/profile', {headers : {Authorization: 'Bearer ' + localStorage.getItem('token')}})
        .then(reponse => {
    ...
    deleteMyAccount() {
        axios.delete(`${process.env.VUE_APP_API_BACKEND_URL}user/${this.userId}/delete`, {headers : {Authorization: 'Bearer ' + localStorage.getItem('token')}})
        .then(reponse => {
            console.log(reponse.data);
            localStorage.clear();
